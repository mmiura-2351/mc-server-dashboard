...
- **MAJOR** (X.0.0): 互換性のないAPI変更（破壊的変更）
- **MINOR** (0.X.0): 新機能追加（後方互換性あり）
- **PATCH** (0.0.X): バグ修正（後方互換性あり）

**例**:
- `1.0.0` → `1.0.1`: バグ修正（パッチ）
- `1.0.1` → `1.1.0`: 新機能（マイナー）
- `1.9.0` → `2.0.0`: 破壊的変更（メジャー）
...
- ✅ マージ前に会話の解決を要求
- ✅ 上記設定のバイパスを許可しない
- ❌ 承認を要求：任意（個人開発のため自動チェックに依存）
- ❌ 直線的な履歴を要求：不要（squash mergeを使用）

### `release/*`ブランチ

- ✅ マージ前にプルリクエストを要求（ホットフィックス用）
- ✅ ブランチ削除を許可しない
- ✅ タグの不変性を要求（タグは削除・上書き不可）
- ✅ ステータスチェックの合格を要求（mainと同じ）
- ❌ 直接プッシュ：リリース準備コミットのみ許可
